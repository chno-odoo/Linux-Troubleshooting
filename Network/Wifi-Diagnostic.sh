...
perform_speed_test() {
    ...
    if [ $speed_test_exit_code -eq 0 ]; then
        ...
        # Extract data from speed test result
        ...
        # Append the CSV data to the global variable
...
summarize_findings() {
    ...
    # python3 ReportCreation/HTMLtoPdf.py "$wifi_json"



    # Ensure jq is installed
    if ! command -v jq &> /dev/null; then
        echo "Error: jq is not installed. Install it using 'sudo apt install jq' (Debian/Ubuntu) or appropriate package manager."
        exit 1
    fi
    operating_system=$(cat /etc/os-release 2>/dev/null | grep PRETTY_NAME | cut -d'"' -f2)
    RFKILL_STATUS=$(rfkill list)
    WIFI_FREQUENCY=$(echo "$link_info" | awk '/freq/{print $2}')

    # JSON generation using jq
    wifi_json=$(jq -n \
        --arg SSID "$SSID" \
        --arg missing_tools "$missing_tools" \
        --arg SIGNAL_THRESHOLD "$SIGNAL_THRESHOLD" \
        --arg QUALITY_THRESHOLD "$QUALITY_THRESHOLD" \
        --arg SIGNAL_STRENGTH "$signal_strength" \
        --arg SIGNAL_LEVEL "$SIGNAL_LEVEL" \
        --arg WIFI_FREQUENCY "$WIFI_FREQUENCY" \
        --arg VPN_ACTIVE "$VPN_ACTIVE" \
        --arg VPN_TYPE "$VPN_TYPE" \
        --arg VPN_INTERFACE "$VPN_INTERFACE" \
        --arg POWER_SAVE "$POWER_SAVE" \
        --arg PING_RESULTS "$PING_RESULTS" \
        --arg download "$download" \
        --arg upload "$upload" \
        --arg RFKILL_STATUS "$RFKILL_STATUS" \
        --arg operating_system "$operating_system" \
        --arg speed_value "$speed_value" \
        --arg interface "$interface" \
        --arg wifi_info "$wifi_info" \
        --arg frequency2 "$frequency2" \
        --arg signal_abs "$signal_abs" \
        --arg tx_bitrate "$tx_bitrate" \
        --arg connection_speed "$connection_speed" \
        --arg csv_data "$csv_data" \
        '{
            "SSID": $SSID,
